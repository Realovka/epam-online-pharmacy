...
/**
 * The type Updating product plant command.
 */
public class UpdatingProductPlantCommand implements Command {
    ...
    public CommandResult execute(HttpServletRequest request) {
        ...
        String plant = request.getParameter(RequestParameter.UPDATING_PRODUCT_PLANT);
        ...
        if (!productValidator.isValidStringParameters(plant)) {
            request.setAttribute(RequestAttribute.PRODUCT_PLANT_ERROR, BundleKey.PRODUCT_PLANT_ERROR);
            return new CommandResult(PagePath.UPDATING_PRODUCT_PLANT, CommandResult.RoutingType.FORWARD);
        }

        try {
            productService.updateProductPlant(productId, plant);
            if (currentPage != 1) {
            ...
            logger.log(Level.ERROR, """
                    ...
                    find list current products or update plant of product """, e);
